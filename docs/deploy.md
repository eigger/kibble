...
대용량 영상 첨부는 **청크 업로드**(8MB 단위)를 사용합니다. 총 파일 상한은 API 환경 변수 `FILE_SIZE_LIMIT_MB`(기본 500)입니다. 업로드는 원본 크기 기준이고, 큰 영상은 서버가 백그라운드에서 720p H.264로 줄입니다(이미 작은 파일은 건너뜁니다). 변환은 업로드 **이후**라, 순간 디스크는 대략 `FILE_SIZE_LIMIT_MB × 이벤트당 첨부 상한(9)`까지 갈 수 있습니다 — `uploads` 볼륨은 그 최대치를 감당해야 합니다. 변환이 끝나면 파일이 줄어듭니다.
