...
복원 아카이브 상한은 `BACKUP_RESTORE_LIMIT_MB`(기본 2047)입니다. 아카이브는 메모리가 아니라 디스크로 흘려 받으므로 이 값을 올려도 API 메모리는 늘지 않습니다 — 제약은 `uploads` 볼륨의 여유 공간입니다. 상한을 넘으면 413으로 막힙니다.

대용량 영상 첨부는 **청크 업로드**(8MB 단위)를 사용합니다. 총 파일 상한은 API 환경 변수 `FILE_SIZE_LIMIT_MB`(기본 500)입니다. 이 값을 올릴 때는 **2047을 넘기지 마세요** — `Attachment.size`가 Prisma `Int`(signed 32-bit, 약 2047MB)라 그 위에서 컬럼이 뒤집힙니다. 복원 상한을 2048이 아니라 2047로 둔 것과 같은 이유입니다. 업로드는 원본 크기 기준이고, 큰 영상은 서버가 백그라운드에서 720p H.264로 줄입니다(이미 작은 파일은 건너뜁니다). 변환은 업로드 **이후**라, 순간 디스크는 대략 `FILE_SIZE_LIMIT_MB × 이벤트당 첨부 상한(9)`까지 갈 수 있습니다 — `uploads` 볼륨은 그 최대치를 감당해야 합니다. 변환이 끝나면 파일이 줄어듭니다.

배포 직후 기존 영상은 `pending`으로 한 바퀴 돕니다. 영상이 쌓인 설치에서는 몇 시간 CPU를 쓸 수 있습니다. 로그는 `[video-transcode]`입니다. `uploads` 볼륨에 `tmp/` 임시 조각이 생길 수 있으며, 24시간 지난 세션은 API가 정리합니다.

영상 첨부의 목록용 **대표 프레임**은 API 이미지에 포함된 `ffmpeg`로 뽑습니다(1프레임 추출). 같은 바이너리로 백그라운드 재인코딩도 합니다. ffmpeg가 없거나 해당 코덱을 못 열면 대표 프레임 없이 원본이 그대로 저장됩니다 — 업로드는 성공합니다. 포스터 실패는 `[videoPoster]`, 변환 실패는 `[video-transcode]`로 남습니다.
...
5. (선택) 더보기 → **백업/복원**(`/backup`) — DB는 **계정·가구·설정만** 다룹니다(반려동물·이벤트·프리셋 행은 아직 포함되지 않습니다). 다만 **업로드된 파일(`uploads/` 아래 사진·영상·펫 사진)은 아카이브에 그대로 담깁니다** — 그래서 아카이브가 첨부 용량만큼 커집니다
