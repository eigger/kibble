...
- **사이드카에는 변하지 않는 정보만 담는다.** 진행량은 `.part` 파일 크기에서 유도한다.
  ...
  디스크가 언제나 정답이다
- **재개할 때 청크 경계까지 잘라낸다.** `appendFile`은 원자적이 아니라 도중에 죽으면
  경계에 안 맞는 꼬리가 남는다. 그대로 이어 붙이면 조용히 깨진 영상이 된다 — 최대 8MB를
  다시 받고 만다 (R72). 다만 `receivedBytes === totalSize`면 자르지 않는다. 마지막 청크는
  경계에 안 맞는 게 정상이고, `complete`를 기다리던 상태이기 때문이다
