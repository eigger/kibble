...
| R89 | 첨부를 OS 백그라운드로 올리기 (Background Fetch·SW) | **기각** | iOS 홈화면 PWA는 탭 JS가 멈추면 전송이 멈춘다. Safari는 Background Fetch가 없다. "앱을 나가지 마세요"는 그 한계의 안내이지, 백그라운드가 아니다. 시트만 닫고 같은 화면에서 올리며, 그만두기를 둔다 | 플랫폼이 홈화면 PWA에서 전송을 이어서 돌릴 때 |
