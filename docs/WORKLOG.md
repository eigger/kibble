...
`lib/uploadGuard.ts` — 업로드가 도는 동안 `beforeunload`를 건다. 카운터라 배치와 개별
파일 양쪽에서 감싸도 안전하다. `uploadEventAttachment`에 걸었으므로 오프라인 큐 재전송을
포함해 모든 업로드 경로가 덮인다.

문구는 넣지 않았다 — 브라우저가 `returnValue`를 무시하고 자기 문구를 쓴다 (R73).
**iOS Safari는 `beforeunload`를 지원하지 않아 거기서는 보호가 없다.**

**확인**

`npm test` api 172 / web 150 / shared 31 통과, `tsc --noEmit`·eslint 통과.
재시작 재개는 "모듈을 다시 import해 메모리 맵이 빈 새 프로세스를 만든다"로 시뮬레이션했다 —
반쯤 올라간 업로드 되살리기, 찢어진 청크 잘라내기, 완성된 파일 보존, 손상된 사이드카,
지운 세션이 되살아나지 않기까지 못박았다.

**다음**

- 실기기에서 재개 확인: 영상 업로드 중 API 재시작 → 같은 이벤트에 **같은 파일을 다시 선택** →
  이어받는지. (재개는 파일을 다시 골라야 한다 — 페이지가 File 객체를 들고 있지 못한다)
- 화면 꺼짐으로 인한 중단은 그대로다. Screen Wake Lock API가 후보지만 배터리와 맞바꾼다
