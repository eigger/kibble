...
| R62 | 서브패스 Caddy 예시를 기본 `Caddyfile` 주석으로 넣기 | **기각** | `update`가 배포 파일 diff를 터미널에 그대로 보여 준다. 실제 설정은 안 바뀌는데 `# handle /kibble*` 주석 블록이 "설정이 바뀌는 것처럼" 보인다. 예시는 `docs/deploy.md`에 둔다 | — |
