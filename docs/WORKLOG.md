...
| R61 | `createImageBitmap`의 `resizeWidth`로 디코드 단계에서 축소해 메모리 스파이크 회피 | **기각** | 디코드 **전에는** 가로/세로 중 어느 쪽이 긴지도, 원본이 1600px보다 큰지도 알 수 없다. `resizeWidth`만 주면 세로 사진에서 긴 변이 그대로 남고, 1600px보다 작은 사진은 억지로 **확대**된다(스펙상 정확히 그 값으로 리사이즈). 대신 `drawImage` 직후 비트맵을 즉시 놓아 최대 점유에서 "원본 + 인코더" 겹침을 없앤다 | 원본 크기를 디코드 없이 읽는 경량 헤더 파서를 들일 만한 이유가 생길 때 |
...
canvas가 HEIC을 디코드하지 못하는 환경(데스크톱 Chrome·구형 WebView)에서는 원본 HEIC이
그대로 올라가 서버 400을 받는다. K-12에 따라 클라이언트가 파일을 막지는 않고, 이제는
**그 한 장만** 실패하고 나머지는 올라간다(R59) — 이 정도면 감수할 만하다고 봤다.
모바일 브라우저는 대부분 HEIC을 디코드한다.
