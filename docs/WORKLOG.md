...
| R77 | 복원 상한을 500MB로 유지 | **기각** | 그 숫자는 아카이브가 사실상 `db.json`뿐일 때 정해진 것이라 의미가 없었다. 이제는 **내보내기는 되는데 복원이 막히는** 상황을 만든다 — 업로드 단건 상한이 150MB라도 누적 첨부는 금방 넘는다. 기본 2GB + `BACKUP_RESTORE_LIMIT_MB` | — |
...
- **복원 상한 500MB → 기본 2GB** (`BACKUP_RESTORE_LIMIT_MB`). 500MB는 아카이브가 사실상
