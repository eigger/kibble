...
| R64 | 영상을 서버에서 ffmpeg로 재인코딩(720p·H.264)해 저장 | **보류** | 효과는 확실하다(15초 4K 클립 56MB → 약 3.7MB). 문제는 무게다 — Node 단일 프로세스라 동기 실행하면 4K 1분 인코딩 동안 API 전체가 멈춘다. 큐 + 워커 + `Attachment` 상태 컬럼 + 마이그레이션 + 변환 중 UI가 한 세트로 따라온다. **Range 지원·포스터 프레임·상한 조정을 먼저 하면 재생 품질과 목록 성능은 해결되고, 디스크는 상한이 상당 부분 막는다** | 위 셋을 하고도 디스크 증가가 실제 문제로 확인될 때 |
| R65 | 브라우저에서 영상을 압축해 올리기 (WebCodecs 등) | **기각** | R63과 같은 함정이고 더 나쁘다. 12MP JPEG를 canvas로 리인코딩하는 것만으로 갤럭시에서 장당 수 초~20초가 걸려 못 썼다 — 비디오 트랜스코딩은 그보다 훨씬 무겁고 기기 편차가 크다. 압축을 한다면 서버다 (R64) | — |
...
| R68 | 영상 상한 500MB를 유지 | **기각** | 상한이 허용하는 크기와 **끝까지 올릴 수 있는 크기**가 달랐다. 실제로 384MB 영상이 8MB 청크로 수십 분 올라가다 끊겨 `tmp/`에 조각만 남았다 — 세션은 메모리에 있어 API 재시작이면 재개도 안 된다. 상한을 150MB로 낮추면 init 단계에서 **바이트를 보내기 전에** 413으로 막혀, 20분을 버리는 대신 곧바로 이유를 안다. 더 필요한 설치는 `FILE_SIZE_LIMIT_MB`로 올린다 | — |
