...
- 영상 전체를 RAM에 복사하지 않는다. 수백 MB `arrayBuffer`는 탭을 멈추고 8초 타임아웃에 걸려 영상만 실패했다. 사진은 되고 영상은 안 되던 이유. 청크는 `slice`로 원본을 읽는다.
- 업로드 `complete`가 ffprobe·포스터 ffmpeg를 기다려 폰이 99%에서 붙잡혔고, 직후 720p 변환이 기본 스레드 수로 2GB CT를 OOM 낼 수 있었다. 포스터·probe는 잡으로 옮기고, ffmpeg는 `-threads 1`.
