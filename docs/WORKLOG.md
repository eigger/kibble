...
| R37 | 퀵 칩 잘림을 양쪽 시작 정렬로 해결 | **기각** | 가운데·아래 밀집이 목적인데 칩이 바깥으로 흩어지고 행마다 들쭉날쭉해진다. 라벨은 각 그룹 왼쪽(기존), 그룹 단위로 가운데에 붙이고 넘침은 칸 안에서만 스크롤 | — |
...
칩을 하단 중앙(엄지·FAB 근처)에 모은다. 글자는 잘리지 않게, 행마다 들쭉날쭉하지 않게. 표시 이름은 **Kibble**(대문자 시작). 패키지·저장소·캐시 키는 소문자 `kibble` 유지.

**원인**

2열에서 가운데로 모으는 것 자체는 맞다. 다만 넘친 칩이 **중앙 경계 쪽으로** 잘렸다. 왼쪽 칸은 스크롤 원점이 왼쪽이라, 가운데에 있어야 할 칩이 잘리고 바깥 칩만 보였다.

**한 일**

- 라벨은 각 그룹 왼쪽 유지(중앙 척추 안은 철회). 왼쪽 칸은 그룹을 `flex-end`, 오른쪽 칸은 `flex-start`로 가운데에 붙인다
- 칩 영역은 내용 폭만큼만 쓰고 칸을 넘기면 그 안에서만 가로 스크롤. 행은 `overflow: hidden`이라 이웃 칸을 침범하지 않는다
- 왼쪽 칸이 넘치면 칩 스크롤을 끝으로 붙여 가운데 쪽 칩이 보이게 한다. `flex-end`만 쓰면 넘친 쪽이 스크롤되지 않는다
- 칩 너비는 짧은 라벨은 공통 `min-width`, 긴 라벨만 늘어남. 한글 2자면 같은 폭이라 열이 맞는다
- 양쪽 시작 정렬로 잘림만 피하는 안은 R37로 기각
