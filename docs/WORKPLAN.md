...
| **관찰 태그는 두 묶음** | 몸(눈꼽·귀지·냄새·피부·털·탈모·치아·잇몸 색·동공·상처) / 증상·행동(긁음·기침·재채기·절뚝임·호흡 이상·울음·이상행동·수면 변화). 9→18개를 한 줄에 나열하면 훑기 어렵다. 활력(ENERGY_3)은 척도라 항목을 "추가"할 자리가 없다 — 울음·이상행동·수면은 태그다 |
| **태그 표현은 진단이 아니라 보이는 것** | "곰팡이"는 진단명(피부사상균)이라 K-16에 걸린다 → **탈모**. "혈색"은 실제로 보는 게 잇몸·점막 색이라 → **잇몸 색**. "수면"은 방향이 없어 → **수면 변화** (R139) |
