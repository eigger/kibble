...
model MedicationCourse {
  ...
  householdId String
  petId       String
  name        String              // "아침약" — 약의 정체가 아니라 자리(슬롯) 이름이어도 된다 (§7.19)
  ingredients String?             // 성분 자유 텍스트. 처방마다 조성이 달라 구조화하지 않는다 (§7.19)
...
- **지난 처방은 사라지지 않는다** — 케어 화면 "지난 처방"에서 이름별로 묶어 기간·성분을 본다. 성분이 바뀌면 "새 처방으로 이어가기"로 갈아탄다 (§7.19)
...
| P1-20 | **홈 = 타임라인 + 하단 입력 바** (§3.7). 상단 오늘 요약 한 줄 | **완료** — 텍스트 입력 P1-22에서 활성화. 반려동물 탭 2마리+. **요약의 내용은 P2-14에서 횟수 → 양·마지막 시각으로 바뀌었다 (§7.17)** |
...
| **P2-14** | **홈 = 오늘 화면** — 오늘 요약을 양·마지막 시각·복약으로 (§7.17) | **완료 (PR #107)** — 별도 "오늘" 페이지는 만들지 않는다(R133). `groupBy(["eventTypeId", "unit"])` + `_sum`·`_max`, 빈 단위는 `defaultUnit`으로 접는다. 표시 규칙은 `EventCategory`·`ScaleType`·`defaultUnit` 기준 (K-8). 누적 통계 카드 제거. 최근 목록 첨부는 이력과 같은 썸네일·라이트박스 |
| **P2-15** | **루틴** — 미리 정한 값(사료 10g, 영양제 3종)을 `/q`에서 1탭 저장 (§7.24) | **완료** — `Routine`/`RoutineItem` 테이블 + `/api/routines` CRUD(K-1 격리 테스트). `/q` 입력 바가 칩 ↔ 루틴 두 패널(세그먼트·옆으로 밀기·기록 탭 재탭)이 되고 마지막 모드를 기기에 기억한다. 저장은 항목마다 `POST /api/events`(K-4) + 같은 `entryId`(§7.22) + 실행취소 토스트. 관리는 더보기 → 루틴. 투약 제외. **기록에서 바로 루틴 만들기는 미구현** |
...
> 요약이 **무엇을** 보여주는지는 §7.17로 바뀌었다(횟수 → 양·마지막 시각). 여기서 정하는 것은 **일 경계**뿐이며 그 결정은 그대로다.
...
### 7.15 첨부 저장 — 내용 해시 + 하드링크

| 결정 | 내용 |
|---|---|
| **중복 첨부는 하드링크로 잇는다** | 저장되는 바이트의 sha256을 `Attachment.contentHash`에 남기고, 같은 가구에 같은 해시가 있으면 파일을 새로 쓰지 않고 `link()`로 이름만 하나 더 만든다. 행은 공유하지 않는다 — `@@unique([path])`와 여러 삭제 경로가 그대로 유지되고, 참조 수는 파일시스템이 센다 (R128) |
| **변환 시도는 집는 순간 센다** | `Attachment.transcodeAttempts`를 `pending → processing` 전이에서 올린다. 실패 시점에 세면 ffmpeg가 컨테이너를 OOM으로 죽였을 때 아예 세지지 않아, 기동마다 되살아나 또 죽는 고리가 된다. 상한 3회를 넘기면 `failed`로 굳힌다 |
| **실패한 영상의 원본은 지우지 않는다** | 변환은 용량 최적화지 첨부의 조건이 아니다. 상한을 다 써도 원본 그대로 남긴다 (K-13, R129) |

### 7.16 백업 내보내기 — 빌드는 요청 밖에서, 다운로드는 링크로

| 결정 | 내용 |
|---|---|
| **아카이브 빌드를 요청에서 뗀다** | `POST /api/backup/export/jobs`가 빌드를 시작하고 즉시 돌아온다. 화면은 `GET /api/backup/export/jobs/:id`로 진행률(단계·복사 바이트)을 물어본다. 예전에는 `GET /export` 하나가 만들면서 흘려보내 새 탭이 분 단위로 빈 화면이었다 (R130) |
| **다 되면 링크를 건다** | `GET /export`는 이제 만들지 않고 **미리 만들어 둔 것만** 흘려보낸다. 티켓에 `jobId`를 싣고, 준비되지 않았으면 409다. 티켓은 60초라 링크를 누르는 순간 발급받는다 |
| **동시에 하나만 만든다** | 빌드 하나가 uploads 한 벌을 복사한다. 둘이 겹치면 디스크가 세 배가 되므로 두 번째 요청은 409로 돌려주고 도는 작업의 진행률을 건넨다 |
| **취소는 플래그로** | 빌드를 밖에서 죽이면 tar가 반쯤 쓴 파일을 남긴다. 취소는 표시만 하고 빌드가 다음 확인 지점(파일 하나 사이)에서 스스로 접으며 치운다. **표시한 작업을 목록에서 바로 빼지 않는다** — 빼면 잠금이 풀려 두 번째 빌드가 시작된다 |
| **`files/`는 사본이 아니라 하드링크다** | `backup_<ts>/files/`는 `UPLOAD_DIR` 안이라 늘 같은 파일시스템이다. 링크는 크기와 무관하게 즉시고 자리를 차지하지 않는다 — 백업 한 번이 원본만큼의 디스크를 더 먹던 것이 없어지고, 중복 제거로 하드링크된 첨부가 아카이브에서 다시 풀리던 것도 없어진다 (R132). 링크가 안 되는 환경에서는 조용히 복사로 되돌아간다 (K-12) |
| **복원도 하드링크를 지킨다** | 아카이브에서 한 아이노드를 가리키던 파일들을 `cp`로 풀면 한 벌씩 새로 쓰인다 — **복원 한 번에 중복 제거가 통째로 되돌아간다.** 아이노드를 기억했다가 두 번째부터 링크로 잇는다. 덮어쓸 때는 기존 파일을 지우고 새로 만든다 — 아이노드에 덧쓰면 그 파일에 걸린 다른 기록의 내용까지 바뀐다 |
| **사본은 tar 직후 버린다** | 아카이브가 나온 뒤로는 쓸모가 없다. 예전에는 다운로드 스트림이 닫힐 때까지 들고 있었다 |
| **진행률의 폭은 압축이 가진다** | 담는 단계가 하드링크가 되면서 순식간에 끝나고 시간은 전부 압축으로 옮겨 갔다. tar가 도는 동안 아카이브 파일이 자라는 크기를 진행률로 쓴다 — 사진·영상은 이미 압축돼 있어 원본과 비슷한 크기로 자라므로 쓸 만한 근사다. verbose 출력을 파싱하지 않는 이유는 GNU tar와 busybox tar의 형식이 다르고, 알고 싶은 것이 파일 수가 아니라 바이트이기 때문이다 |
| **여유 공간은 원본의 1배** | `files/`가 자리를 차지하지 않으므로 아카이브 한 벌만 보면 된다. 2배로 두면 아카이브를 담을 공간이 충분한 서버에서도 백업이 거부된다 |
| **받지 않은 아카이브의 수명** | 작업은 2시간 뒤 목록에서 빠진다 — `sweepStaleBackupWorkspaces()`가 파일을 걷는 나이와 같게 맞춘 것이다. 더 오래 들고 있으면 파일이 없는데 화면만 "받을 수 있다"고 말한다 |

### 7.17 홈 = 오늘 화면 — 횟수가 아니라 양과 마지막 시각

§3.7의 "오늘 · 사료 3 · 물 2"는 **행위 횟수**다. 실사용에서 이 숫자는 답이 되지 않았다 — "사료 3"은 40g씩 세 번인지 대부분 남긴 건지 구분하지 못하고, 하루에 여러 번 앱을 여는 사람이 실제로 묻는 것은 "몇 번 줬나"가 아니라 **"줬나? 언제?"**이기 때문이다.

| 결정 | 내용 |
|---|---|
| **별도 "오늘" 페이지를 만들지 않는다** | 홈이 곧 오늘이다. 페이지를 나누면 오늘 정보가 두 곳이 되고(R40과 같은 구조), 하단 탭 5칸이 이미 차 있어 새 탭은 "더보기"에 묻힌다 — 하루 여러 번 보는 화면이 거기 있을 수 없다 (R133) |
| **오늘 요약은 양·마지막 시각·복약을 낸다** | `todaySummaryForPet`이 `_count`만이 아니라 **단위별 `_sum(quantity, quantityOffered)`·`_max(occurredAt)`·마지막 척도값**을 함께 낸다. 사료는 "제공 120g / 섭취 90g", 물은 "250ml", 배변은 횟수 + 마지막 변 상태 |
| **"마지막 언제"가 합계보다 먼저다** | 카드마다 상대 시각(방금 / 40분 전 / 3시간 전)을 붙인다. 확인하러 여는 사람에게 답이 되는 값은 이것이다 |
| **표시 규칙은 이벤트 타입 키가 아니라 `EventCategory`·`ScaleType`·`defaultUnit`으로 정한다** | K-8. 타입을 하나 추가할 때 홈 코드를 고쳐야 하면 모델이 틀린 것이다. 양이 있으면 합계, 척도가 있으면 마지막 값, 둘 다 없으면 횟수. **예외: HEALTH의 양은 측정값이라 합계가 아니라 마지막 값**(체중·체온, §7.23) |
| **빈 단위는 그 타입의 기본 단위다** | `unit`이 비어 있으면 `EventType.defaultUnit`으로 읽어 같은 칸에 더한다 — 앱의 다른 곳(`formatEventDetailLine`·`parseEntry`)이 모두 그렇게 읽는다. 여기서만 null을 별도 단위로 두면 **단위를 안 보내는 자동 입력과 화면 입력이 섞인 날 합계가 통째로 사라진다** |
| **정말로 섞인 날은 합계를 내지 않는다** | 같은 타입에 g과 개가 같이 들어온 날 둘을 더하면 거짓말이 된다. 그런 타입은 조용히 횟수로 떨어뜨린다 (K-12 — 입력을 거부하지는 않는다) |
| **누적 통계("누적 기록 42건 · 기록한 날 5일")를 홈에서 뺀다** | P1-27b의 초보자 유도 장치였고 습관이 붙은 뒤에는 값이 매일 커지기만 할 뿐 아무 행동으로도 이어지지 않는다. **안내 문장(`journalInsightMessage`)은 남긴다** — 그쪽이 §3.8이 의도한 유도이고, 3일이 지나면 스스로 사라진다 |
| **추세는 계속 `/analytics`가 가진다** | 축이 다르다. 홈은 "오늘", 분석은 "기간". 홈에 기간 비교를 들이지 않는다 |

### 7.18 칩은 타입, 세부는 태그 — 관리(`care`) 타입과 관찰 태그 확장

빠른 기록 칩 바는 모바일에서 이미 꽉 차 있다. 그런데 기록하고 싶은 항목은 계속 늘어난다 — 관찰에 이빨·털·상처·동공·울음·수면, 케어에 눈·귀·발톱·목욕·빗질. **항목마다 칩을 만들면 끝이 없다.**

| 결정 | 내용 |
|---|---|
| **칩은 이벤트 타입 하나당 하나, 세부는 상세 시트의 태그** | 칩 수의 상한은 타입 수(~12)로 고정된다. 항목이 늘어도 칩은 늘지 않는다. `/q`는 어떤 칩이든 상세 시트를 거쳐 저장하므로 "칩 → 태그 1탭 → 저장"은 다른 기록과 1탭 차이다 (R136) |
| **`grooming`(그루밍)을 `care`(관리)로** | 태그: 양치 · 눈 닦기 · 귀 청소 · 발톱 · 목욕 · 빗질 · 만져주기. 이름은 "케어"가 아니라 **"관리"** — 하단 탭 "케어"(투약·리마인더)와 겹치지 않도록 (R138) |
| **`dental`(양치)을 `care` + 태그 `dental`로 흡수** | 칩 수 net 0 — 가장 붐비는 HEALTH 줄에서 양치가 빠지고 CARE 줄에 관리가 들어간다. 양치가 매일이면 하루 1탭이 늘지만 태그 맨 앞에 두어 손가락 자리를 고정한다. 코드 의존은 시드 템플릿뿐이라 오늘 카드·분석·케어 탭은 안 건드린다 |
| **기존 양치·그루밍 기록은 소속만 바뀐다** | `energy`→`observation`과 같은 시드 마이그레이션(`mergeSystemEventType`). 이벤트·프리셋·리마인더·토큰·가구 별칭을 `care`로 옮기고 옛 타입 행은 보관(archive). 양치 기록은 "관리 · 양치"로 읽힌다. 삭제되는 것은 없다 |
| **관찰은 "봤다", 관리는 "했다"** | 같은 부위라도 표현을 다르게 둔다 — 관찰은 눈꼽·귀지, 관리는 눈 닦기·귀 청소. 케어 행동을 관찰 시트에 섞으면 CARE 카테고리·이력 필터·"마지막 목욕이 언제였지"가 전부 깨진다 (R137) |
| **관찰 태그는 두 묶음** | 몸(눈꼽·귀지·냄새·피부·털·탈모·곰팡이·치아·잇몸 색·혈색·동공·상처) / 증상·행동(긁음·기침·재채기·절뚝임·호흡 이상·울음·이상행동·수면 변화·**이물섭취**). 9→19개를 한 줄에 나열하면 훑기 어렵다. 활력(ENERGY_3)은 척도라 항목을 "추가"할 자리가 없다 — 울음·이상행동·수면·이물섭취는 태그다. **이물섭취**는 진단이 아니라 보호자가 본 사실(무엇을 삼켰는지는 메모로) — K-16 |
| **태그 표현은 진단이 아니라 보이는 것** | "곰팡이"는 진단명(피부사상균)이라 K-16에 걸린다 → **탈모**. "혈색"은 실제로 보는 게 잇몸·점막 색이라 → **잇몸 색**. "수면"은 방향이 없어 → **수면 변화** (R139). 이후 실사용에서 잇몸과 별개로 **귀·코·피부 전반의 혈색**을 보는 일이 있어 **혈색**을 따로 추가했다 — 잇몸 색을 대체하지 않는다. **곰팡이**도 같은 이유로 추가 — 탈모와 별개로 원형 각질·딱지를 보호자가 그렇게 부른다. 태그는 보호자의 말이지 진단이 아니다 |
| **`ear_smell`→`smell`은 파서 별칭으로 흡수** | 저장된 옛 slug를 읽을 때만 현재 id로 바꾼다. 실기록이 있으므로 raw slug가 화면에 노출되면 안 된다 |

### 7.19 처방 이력 — 과정 목록이 곧 성분 변천 이력

약을 꾸준히 먹이는데 성분이 조금씩 바뀐다. "언제부터 언제까지 어떤 성분을 먹였나"를 봐야 하는데, 종료한 처방은 케어 화면에서 사라지고 타임라인은 회차 단위라 과정 단위 질문에 답하지 못했다.

| 결정 | 내용 |
|---|---|
| **처방 이력 = `MedicationCourse` 목록.** 새 테이블·이벤트 없음 | 처방 1건 = 과정 1개(§3.10)는 이미 확정이다. 성분이 바뀌면 종료 + 새 처방이므로 **과정 목록이 곧 성분 변천 이력**이다. 타임라인에 "처방 시작/종료" 이벤트를 복제하면 과정 행과 두 곳이 어긋난다 (R142) |
| **성분은 `MedicationCourse.ingredients` 자유 텍스트** | `dosage`와 같은 결(R127). 조회 대상이 아니다. `Product(MEDICATION)` 참조는 하지 않는다 — 조제약은 처방마다 조성이 달라 처방 하나마다 제품을 만들어야 하고, 카탈로그가 처방 기록으로 오염된다 (R141) |
| **"지난 처방" = `archivedAt` 있음 OR `endDate` 경과** | 사라지는 경로가 둘인데 사용자에게는 하나로 보여야 한다. 표시하는 종료 시점은 **마지막 복약 시각 → `endDate` → `archivedAt`** 순 — "언제까지"의 정답은 처방일이 아니라 마지막에 먹인 날이다 |
| **이름은 계보의 키** | 사용자는 "아침약"·"저녁약"처럼 자리 이름을 쓴다. 이름을 매번 다르게 지으라는 건 마찰이고 약의 정체도 아니다. 지난 처방은 **이름별로 묶어** 그 안에서 기간·성분을 나열한다 — 같은 이름이 반복되는 것이 오히려 정보("아침약이 이렇게 바뀌어 왔다")가 된다. 성분 변경 마커는 **같은 묶음의 직전 행**과 비교한다. 이름이 달라 갈라진 묶음은 자동으로 합치지 않는다 — 지난 처방도 이름을 고칠 수 있다 |
| **"새 처방으로 이어가기"는 기존 처방 위의 버튼** | 전역 메뉴가 아니다. 아침약 상세에서 누르면 아침약 값이 채워진 새 처방 폼이 열리고, 저장하면 이전 처방은 같은 트랜잭션에서 종료된다(`continuesCourseId`). 어느 처방에서 출발했는지가 화면에 명시된다. "처방 추가"에서 같은 이름을 치면 묻는 방식은 암묵적이고 이름을 살짝 다르게 치면 못 잡아 기각 (R143) |
| **간트형 기간 시각화는 보류** | 목록으로 실사용해 보고 필요하면 |

### 7.20 화장실 관리 — `care` 태그 + 위생용품 연결

화장실 청소·모래 보충·모래 갈이를 기록하고 싶은데, "어느 모래를 넣었나"가 같이 남아야 다음 구매·소진 계산으로 이어진다. 시드에 `litter_change` 타입(CAT 전용)이 있었지만 어떤 프리셋 템플릿에도 없어 **칩 바에 나온 적이 없다** — 사실상 고아 타입이었다.

| 결정 | 내용 |
|---|---|
| **화장실 일은 `care`(관리)의 태그 묶음이다.** 새 타입·새 칩 없음 | §7.18 그대로 — 칩은 타입 하나당 하나, 세부는 태그. 화장실 청소는 "봤다"가 아니라 "했다"라 관리 자리가 맞다(R137). 매일 하는 청소는 태그 프리셋 기억(`eventDetailPrefs`)이 마지막 선택을 되살려 추가 탭이 0~2회다 |
| **태그는 종별로 보인다** | `EventDetailTag.species?: Species[]` — 없으면 전 종. 모래 보충·모래 갈이는 CAT·OTHER, **패드 교체**는 DOG·OTHER, 청소·세척은 전 종. 피커만 거른다 — 저장된 slug는 종과 무관하게 읽는다 (K-13). `EventType.species`와 같은 결의 allowlist |
| **관리 태그도 묶음 소제목을 얻는다** | 몸(양치·눈 닦기·귀 청소·발톱·목욕·빗질·만져주기) / 화장실(청소·모래 보충·모래 갈이·패드 교체·세척). 관찰과 같은 피커 구조 |
| **`litter_change` 시스템 타입은 `care` + 태그 `litter_change`로 합친다** | `dental`→`care`와 같은 `mergeSystemEventType`. 시드 목록에서 빼야 한다 — 루프의 `seedUpdate`가 `archivedAt: null`로 되살리기 때문 |
| **관리 기록은 제품(`HYGIENE`)을 잇는다** | `product-suggestions`에 `care`→`HYGIENE`. 등록 제품 칩 줄이 관리 시트에 뜬다 — 모래 갈이엔 모래, 목욕엔 샴푸, 양치엔 치약. `Product`에는 컬럼을 더하지 않는다 |
| **`productName`은 태그 CSV로 남고 제품은 `productId`로만** | 태그 타입(`vomit`·`observation`·`care`)에서 `productName`은 이름이 아니라 slug 목록이다. 따라서 (1) 제품을 골라도 태그를 건드리지 않는다, (2) 서버가 빈 `productName`을 제품 이름으로 채우지 않는다(`TAG_VALUED_PRODUCT_NAME_KEYS`), (3) 표시는 태그 라벨 뒤에 제품 이름을 붙인다 — 대체하지 않는다 |
| **제품 이름 제안·자주 쓰는 이름은 관리에 주지 않는다** | `frequent`는 `productName` 빈도라 slug 문자열을 셀 뿐이고, `lastProduct` 자동 선택은 지난 관리(모래)와 이번 관리(양치)가 다른 일이라 엉뚱한 제품을 붙인다. `care`는 `activeProducts`만 받고 사용자가 1탭으로 고른다 |
| **파서 별칭** | `care`에 모래·모래갈이·패드 추가. 세부 별칭은 `KEYWORD_TAGS`에서 태그 slug까지 붙는다 — 모래갈이→`litter_change`, 패드→`pad_change`. **모래**는 보충·갈이가 갈려 slug를 붙이지 않는다(관리로만). "화장실"은 넣지 않는다 — 배변 문장("화장실에서 똥")에서 오탐 |
| **보류** | "마지막 모래 갈이 N일 전"(홈·케어), 모래 소진일 계산(`weightG`는 R104로 위생용품에 안 뜬다), 같은 태그의 지난 제품 자동 선택 — 실사용 뒤 |

### 7.21 관리(`care`) 환경 태그 — 물 갈이·식기 세척·가구 위치 변경

몸을 직접 손대는 일(양치·목욕)도, 화장실 일도 아닌 "주변을 정돈한" 기록이 자리가 없었다 — 마실 물을 갈아줬는지, 밥그릇을 씻었는지, 가구를 옮겼는지. 이것도 "봤다"가 아니라 "했다"라 관리(`care`) 자리다(§7.18).

| 결정 | 내용 |
|---|---|
| **`care`에 세 번째 태그 묶음 환경을 추가** | 몸(§7.18) / 화장실(§7.20) / **환경**(물 갈이·식기 세척·가구 위치 변경). 새 타입·새 칩 없음 — §7.18 그대로 |
| **"물 갈이"** — "물그릇 교체"가 아니다 | 그릇은 그대로고 안의 물만 새로 간다. **모래 갈이**(모래통이 아니라 모래를 간다)와 같은 결로 이름을 맞췄다 (R152) |
| **FEEDING `water`(급수 기록)와는 다른 축** | `water`는 얼마나 줬는지(급수량, ml)를 기록하고, 관리 태그 "물 갈이"는 위생 유지(오래된 물을 새로 갈았는지)를 기록한다. 라벨을 "물 갈이"(복합어)로 두어 파서 별칭 "물"(FEEDING) 하나만으로는 안 걸리게 했다 |
| **가구 위치 변경은 위생이 아니라 환경 변화 기록** | 물 갈이·식기 세척과 목적이 다르다 — "가구를 옮긴 뒤로 화장실 실수가 늘었나"처럼 나중에 관찰 기록과 대조하려는 용도. 그래도 "정돈했다"는 관리 행위라 같은 묶음에 둔다 |
| **파서 별칭은 이번엔 넣지 않는다** | §7.20 화장실 태그도 처음엔 태그만 넣고 별칭은 리뷰에서 추가했다(R151). 이번에도 태그부터 넣고 실사용 후 판단 |

### 7.22 한 번에 여러 제품 — N개 이벤트를 `entryId`로 묶는다

영양제 몇 가지를 한 그릇에 섞어 먹이는데 기록은 제품마다 칩 탭 → 제품 바꿈 → 양 → 저장을 반복해야 했다. 사료(습식+건식+토퍼)·간식도 같다. `Event`는 `productId`·`quantity`·`unit`이 각 1개인 평평한 행이라 한 행에 여러 제품이 들어갈 자리가 없다.

| 결정 | 내용 |
|---|---|
| **제품 N개 → 이벤트 N건, 같은 `entryId`** | 시트에서 등록 제품을 여러 개 고르고 항목마다 양·단위를 적으면 클라이언트가 이벤트를 N건 만든다. `Event.entryId`는 원래 "한 번에 쓴 글에서 나온 여러 이벤트를 묶는다"(§4 데이터 모델)는 뜻이라 텍스트 파싱이 쓰던 것을 시트가 같이 쓰는 것뿐이다. **스키마 변경 없음** |
| **하나만 고르면 지금과 같다** | 이벤트 1건, `entryId`도 붙지 않는다. 기존 소비자(제품 상세 최근 기록·마지막/자주 쓰는 제품·이력 필터·오늘 요약 단위 합계·백업·오프라인 큐)는 행마다 FK·양·단위가 그대로라 **한 줄도 안 바뀐다** |
| **대상은 영양·사료·간식** (`meal`·`treat`·`supplement`) | `PRODUCT_NAME_EVENT_KEYS` 중 상비(`remedy`)는 뺀다 — 처방·투약 축(§7.19)과 얽혀 있어 따로 본다. 관리(`care`)는 태그가 주고 제품은 부속이라 대상이 아니다 |
| **자식 테이블(`EventItem`)은 만들지 않는다** | 의미상 "이벤트 1건 = 1회 급여"가 맞지만, 실배포·실기록이 있는 상태에서 기존 단일 제품 이벤트(컬럼)와 새 이벤트(items)의 이중 표현이 생기거나 전량 이행이 필요하다. `createEvent`·PATCH·select·표시·제품 최근 기록·통계·백업·오프라인 큐·공유 타깃까지 전부 손대야 한다. `entryId` 묶음이 같은 질문("그때 뭘 얼마나 같이 먹였나")에 답한다 (R153) |
| **`payload` JSON에 항목 배열을 넣지 않는다** | JSON 안의 `productId`에는 FK가 없어 제품 보관·삭제에 끊기고, K-1 스코프 쿼리·제품별 집계가 전부 우회된다 (R154) |
| **다음번은 지난 세트가 그대로 열린다** | `product-suggestions`의 마지막 제품 하나를 **마지막 묶음의 항목 배열**(`lastItems`)로 넓힌다 — 마지막 이벤트에 `entryId`가 있으면 그 묶음 전부, 없으면 그 한 건. 서버 값이라 기기를 옮겨도 같다. 매일의 흐름은 칩 1탭 → 어제 세트가 체크된 채 열림 → 저장 |
| **저장은 건별 POST, `dedupeKey`는 항목별** | 오프라인 큐가 건별이라 그대로 맞고, 중간 실패는 dedupe로 재시도가 안전하다. 첨부·메모는 첫 이벤트에만 붙는다. 트랜잭션 원자성이 필요해지면 `POST /events/batch`(내부는 `createEvent()` 반복, K-4)를 얹는다 — 지금은 안 만든다 |
| **둘째 이후를 먼저 만들고 첫 제품을 마지막에** | 타임라인은 같은 시각이면 id 내림차순(나중 것이 위)이라 이 순서여야 시트에서 보던 대로(첫 제품·메모·첨부가 맨 위) 읽힌다. `lastItems`도 같은 순서(id 내림차순)로 읽어 첫 항목이 다시 첫 칸이 된다. 중간에 실패해도 이미 만들어진 건은 타임라인에 넣는다 |
| **첫 칸이 비었는데 둘째가 있으면 둘째가 첫 칸으로** | 저장 시점에 승격한다 — 안 그러면 내용 없는 이벤트가 한 건 앞에 붙는다 |
| **타임라인은 행을 유지하고 묶음만 표시한다** | 같은 `entryId`의 연속 행은 두 번째부터 시각을 비우고 위 행에 붙인다. 수정·삭제는 행 단위 그대로 — 묶음 삭제는 필요해지면 |
| **홈 오늘 요약은 그대로** | §7.17 단위 합계는 제품이 달라도 같은 단위면 더한다("영양 3 · 4.5g"). 묶음을 1회로 세는 조정은 실사용에서 필요해지면 |

### 7.23 체온(`temperature`) — 측정값은 더하지 않는다

아플 때 아침·저녁으로 체온을 재는데 적을 자리가 없었다. 체중과 같은 결의 **측정값** 타입이다.

| 결정 | 내용 |
|---|---|
| **시스템 타입 `temperature`** | HEALTH, `defaultUnit` `°C`, 아이콘 `thermometer`, sortOrder 85(체중 80 다음). 별칭 체온·열. 프리셋 템플릿에 체중 다음으로 넣는다(스타터 아님) — 기존 반려동물은 홈 진입 시 `ensurePresetsForPet`이 칩을 보충한다. 마이그레이션 없음 (K-8) |
| **시트는 체중과 같다** | 값 한 칸(0.1 단계, 자리표시 38.5), 단위 칸 없음, 메모. 파서는 `38.5도`·`38.5℃`·`38.5°C` 한 줄을 체온으로 읽는다 (`WEIGHT_INLINE_RE`와 같은 자리) |
| **홈 카드: HEALTH + 단위 = 측정값 → 마지막 값** | §7.17의 "양이 있으면 합계"는 급여량엔 맞지만 측정값엔 거짓말이다 — 아침 38.6 · 저녁 38.9를 재면 "77.5°C"가 된다(체중도 두 번 재면 같다). 규칙은 여전히 **`EventCategory`·`ScaleType`·`defaultUnit` 세 필드**로만 정한다: HEALTH 카테고리에서 양이 있으면 그날의 **마지막 값**을 보여준다. 타입 키는 보지 않는다 (K-8) |
| **분석: 체온 추이 선 그래프 + 최근 체온** | 체중 추이와 같은 자리. Y축은 0이 아니라 **데이터 범위 ±0.5**에서 시작한다 — 37~40°C를 0부터 그리면 평평한 선이 된다 |
| **정상 범위 참고선·"열" 판정은 넣지 않는다** | K-16. 종별 정상 범위는 출처를 밝히고 별도 콘텐츠로 검토한 뒤에 (§8 "관찰 기준 문구가 진단으로 읽힌다") |
| **측정 방법은 태그 하나** — 직장(항문) / 귀 / 겨드랑이 / 비접촉 | 종별로 방법이 다르지 않고 방법이 여럿이다. 직장이 기준이고 나머지는 보호자가 실제로 쓰는 방식(조류·파충류의 총배설강도 직장 태그). 종 필터 없음. **한 번에 하나**만 고른다(`EXCLUSIVE_TAG_TYPES`). 값 보정·해석은 하지 않는다 (K-16) — 방법이 섞이면 선이 거짓말할 수 있어 **그래프 툴팁에 방법을 같이** 보여준다. 방법별로 선을 나누는 건 실기록이 쌓인 뒤 |
| **체온계는 `DEVICE` 제품을 `productId`로** | 관리가 위생용품(`HYGIENE`)을 다는 것과 같은 구조(§7.20): `productName`은 태그 CSV, 기기는 `productId`. 타임라인은 "38.5°C · 직장(항문) · 브라운 체온계" |
| **마지막 방법·기기를 그대로 다시 연다** | 관리는 지난 관리(모래)와 이번 관리(양치)가 다른 일이라 자동 선택을 뺐지만(R149), 체온은 하는 일이 하나라 마지막 기록이 다음번에도 맞다 — R149가 남긴 재검토 조건이 이 경우다. `LAST_TAGS_AND_PRODUCT_KEYS`. 자주 쓰는 이름은 세지 않는다(태그 CSV) |
| **기기가 값을 직접 보내는 건 이미 된다** | `ApiToken` 자동 입력(§3.6)으로 `POST /api/events`에 `quantity`·`unit: °C`. 브라우저 Web Bluetooth로 체온계를 직접 읽는 건 iOS Safari 미지원·반려동물용 BLE 체온계 희소라 하지 않는다 (R156) |

### 7.24 루틴 — 미리 정한 값으로 1탭 저장

P1-21/P1-25의 "칩 1탭 POST"는 케어 일지(`4388c2d`)에서 시트 경유로 바뀌었고, 이후 §7.18·R136이 "`/q`는 어떤 칩이든 시트를 거친다"를 전제로 굳었다. 시트는 마지막 묶음(`lastItems`)을 되살려 주지만 그래도 매번 2탭이고, "마지막"은 한 번 다르게 기록하면 바뀐다. 매일 똑같은 기록(사료 10g, 영양제 3종, 물 5.5)은 **고정된 정의를 1탭**으로 넣을 자리가 필요하다.

| 결정 | 내용 |
|---|---|
| **`Preset`에 얹지 않고 `Routine` + `RoutineItem` 테이블** | `Preset`은 (가구·개체·타입)당 활성 1행이고 §7.18이 "칩은 타입당 하나"로 못 박았다. "사료 10g"과 "사료 5g", "영양제 3종 한 번에"는 거기 못 들어간다. 유니크를 풀면 `ensurePresetsForPet`·칩 바·토큰 스코프가 흔들린다. 항목은 `eventTypeId` + `presetId`(같은 타입의 칩 — 이벤트에 실려 타임라인 라벨이 칩과 같아진다) + `productId`/`productName` + `quantity`/`unit` |
| **저장은 항목마다 `POST /api/events`** (K-4) | §7.22와 같은 규칙 — 둘 이상이면 같은 `entryId`, 항목별 `dedupeKey`, 뒤에서부터 만든다. 오프라인 큐가 건별이라 그대로 탄다(R155). `occurredAt`은 누른 시각. 실행취소 토스트는 N건을 한 번에 지운다 |
| **`/q` 입력 바를 두 패널로 — 칩 / 루틴** | 즉시 저장되는 버튼과 시트를 여는 칩이 한 바에 섞이면 매번 "이건 바로 들어가나?"를 생각해야 한다(사용자 지적). 한 번에 한 모드만 보인다. 루틴 패널은 채움색 버튼 + 요약("로얄캐닌 10g · 영양제 · +1") 줄바꿈 그리드. **루틴이 0개면 세그먼트도 패널도 없다** — 지금 화면과 같다 |
| **바꾸는 길 셋: 세그먼트 탭 · 옆으로 밀기 · 기록 탭 재탭** | 세그먼트(`role=tablist`)가 진짜 스위치이고 나머지는 지름길이다. 밀기는 CSS `scroll-snap`(패널 둘, 각 100%)이라 제스처 코드가 없다 — 칩·루틴 그리드가 둘 다 줄바꿈이라 바 안에 가로 제스처를 뺏는 요소가 없다. 하단 내비의 기록 탭은 `/q`에서만 `preventDefault` + 윈도 이벤트로 토글하고, 시트가 열려 있으면 무시한다. 세그먼트 강조는 `IntersectionObserver`가 스크롤 위치를 따라가므로 드래그 중간에 손을 떼도 정직하다 |
| **마지막 모드를 기기에 기억** (`localStorage`) | 매번 칩으로 돌아가면 루틴을 주로 쓰는 사람이 `/q`에서 항상 2탭이 된다. 기억하면 앱 열기 → 기록 탭 → 버튼 = 1탭 |
| **투약은 루틴에 넣지 않는다** | 처방·회차 슬롯 선택이 끼어 1탭이 안 된다. 서버가 `medication` 타입 항목을 거부한다. 케어 탭이 이미 그 일을 한다 |
| **관리는 더보기 → 루틴** | `/routines` — 이름 + 항목(칩·제품·양·단위) 목록. 항목의 제품은 등록 제품에서 고른다. **기록에서 "루틴으로 저장"은 이번엔 안 만들었다** — 상세 시트 메뉴 한 줄이면 되고, 실사용에서 폼이 귀찮다고 확인되면 |
| **이름은 "루틴"** | "빠른 기록"은 `/q` 탭 이름이라 겹친다 |
