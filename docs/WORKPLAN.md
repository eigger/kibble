...
| **저장은 건별 POST, `dedupeKey`는 항목별** | 오프라인 큐가 건별이라 그대로 맞고, 중간 실패는 dedupe로 재시도가 안전하다. 첨부는 첫 이벤트에만 붙는다. 트랜잭션 원자성이 필요해지면 `POST /events/batch`(내부는 `createEvent()` 반복, K-4)를 얹는다 — 지금은 안 만든다 |
