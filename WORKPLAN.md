...
- 근거: `stash`(v0.7.5) · `garage` · `drop` 코드 실사 + [`docs/scenarios.md`](docs/scenarios.md) + 기존 메신저 일지 실사용 분석(§3.7)
...
- 현재 상태: Phase 0 진행 중. 문서·스펙 확정 단계. 애플리케이션 코드 없음.
...
> - r4: 미결 항목 확정 (§7)
> - r5: 시나리오 검증 반영. 인쇄 QR을 1급에서 **보류**로. 근거는 [`docs/scenarios.md`](docs/scenarios.md)
...
> 전면 근거: [`docs/scenarios.md`](docs/scenarios.md). 시나리오 11개(S1~S11) × 경로 8개(A~H)를 대조했다.
...
- **Phase 1 산출물은 `docs/api.md` 하나** — 엔드포인트 + curl 예제. 특정 플랫폼 가이드는 쓰지 않는다.
...
| ID | 항목 | 담당 | 완료 조건 |
|---|---|---|---|
| P0-01 | 3일간 수기 기록 → **프리셋 후보 확정** (8개 이하) + `docs/scenarios.md §2` 빈도 실측 교체 | 사람 | 빈도가 붙은 프리셋 목록 |
| P0-02 | **공개용 기본 `EventType` 시드 + 종별 기본 프리셋 템플릿** 확정. **시작 3개(`isStarter`)와 척도(`scaleType`) 지정 포함** | 사람 + 에이전트 | **`docs/seed-event-types.md` 초안 완료 — 리뷰 필요.** 개인 사용 패턴이 아니라 일반 공개 기본값 (§7.2) |
...
| P0-04 | 시나리오 검토 (온보딩~운영, 설계 정합성) | **완료** | [`docs/scenarios.md`](docs/scenarios.md) |
| P0-05 | PWA 홈 화면 추가 + 오프라인 동작 확인 | 사람 | Android / iOS standalone 동작 |
| P0-06 | 저장소명·npm 패키지명 충돌 확인 | **완료** | [`docs/package-names.md`](docs/package-names.md). GitHub `eigger/kibble`, `@kibble/*` npm 가용 |
...
| P0-09 | **파싱 벤치마크 세트 구축** — 기존 일지에서 실제 문장 100개를 뽑아 (시각·수량·타입) 정답을 붙인다 | 사람 | 개인 100문장은 로컬만. **공개 합성 세트**: [`docs/parsing-benchmark-public.md`](docs/parsing-benchmark-public.md) (~25케이스). 목표치는 P0-09 본문 완료 후 확정 |
...
| P1-12 | **`createEvent()` 단일 서비스 함수** | 서비스 밖에서 이벤트를 만드는 Prisma 호출 0건 |
| P1-13 | `POST /api/events` + 목록 / 수정 / 소프트삭제. **세션·토큰 양쪽 인증 + `dedupeKey` + 빈 본문 처리** (§3.6) | curl 한 줄로 기록이 들어간다. 같은 `dedupeKey` 재전송이 1건으로 |
| P1-14 | **`Preset` CRUD + 숨기기/순서 + `aliases`** | 퀵 칩 길게 누르기 → 숨기기 |
| P1-15 | **한국어 파싱 서비스 (규칙 기반 최소판)** — 시각·수량·타입/별칭, 줄 단위 분해 | 순수 함수 + 단위 테스트. **실패 시 `NOTE`로 폴백**(예외를 던지지 않는다) |
| P1-16 | 다중 첨부 (`Attachment` 여러 장) + 이미지 파이프라인 | 한 이벤트에 사진 9장 |
| P1-17 | 소프트삭제 퍼지 잡 (`trashPurge` 이식) | 크론 동작 확인 |
| P1-18 | `docs/api.md` — 엔드포인트 + curl 예제 | 특정 플랫폼 가이드는 쓰지 않는다 |
...
| P1-20 | **홈 = 타임라인 + 하단 입력 바** (§3.7). 상단 오늘 요약 한 줄 | 메신저와 같은 구조. 반려동물 탭은 **2마리 이상일 때만** 렌더 |
| P1-21 | **퀵 칩 1탭 기록** + 실행취소 토스트 3초 | 평상시 반복 기록이 1탭 |
| P1-22 | **자유 텍스트 입력 + 파싱 제안 칩** | 쓴 대로 저장되고 `rawText` 보존. 파싱 결과는 칩으로 보여주고 탭하면 수정. **거부당하는 입력이 없다** |
...
| P1-27a | **시작 3개 + "더보기"** (G-1) | 온보딩 직후 퀵 칩이 3개. 나머지는 접혀 있다 |
...
동시에 [`docs/scenarios.md §6`](docs/scenarios.md)의 예상 마찰 F1~F9를 실측과 대조한다.
...
| **P2-08** | **지도 프로바이더 설정** (`GET /api/map/providers` + 키 입력 UI) | garage `mapProviders.ts` + `integrations/page.tsx` 이식. 키는 `Setting`에. **키가 없으면 §3.9 기능 전체가 숨는다** |
| **P2-09** | **상호(키워드) 검색 → `Contact` 자동 생성** | garage에 없는 신규. Kakao `services` 장소 검색. 이름·주소·좌표·상세 URL 저장 |
| **P2-10** | **`Contact` CRUD + 지도 표기 + 내비 실행 버튼** | garage `LastLocationMap` · `deepLinks.ts` · `NavLaunchButtons` 이식. `appname`만 kibble로 |
| **P2-11** | **병원 방문 이벤트에 `Contact` 연결** + 병원별 방문 이력·비용 집계 | `Event.contactId`를 타는 쿼리 |
...
특정 연동을 구현하지 않는 것이 핵심이다 — API 하나면 HA·iOS 단축어·ESPHome·Node-RED·curl이 전부 붙는다. Phase 1 산출물은 `docs/api.md` 하나이며 플랫폼별 가이드는 쓰지 않는다. 이 결정으로 마찰 1~3위가 전부 Phase 1에서 열리고, **게이트가 설계 전체를 측정하게 된다.**
...
| **다중 기기 작업 중 계획서가 갈라진다** | 여러 장소에서 서로 다른 전제로 작업 | `WORKPLAN.md`가 단일 진실 원천. 결정이 바뀌면 **코드보다 먼저** 이 문서를 고치고 push. 세션 종료 시 [`docs/WORKLOG.md`](docs/WORKLOG.md)에 기록 ([`CLAUDE.md`](CLAUDE.md)) |
...
17. **K-17**: **세션을 마칠 때 [`docs/WORKLOG.md`](docs/WORKLOG.md)에 기록한다.** 확정·변경한 결정과 근거, **기각한 안과 사유**(§1 표), 다음 세션이 이어받을 것. 여러 장소로 작업이 쪼개지므로 기록하지 않으면 같은 논의를 반복한다. 순서는 `WORKPLAN` → (`PROJECT §4`) → `WORKLOG`.
