...
- 현재 상태: kibble 저장소 비어 있음 (git 초기화 전). Phase 0 진행 중.
...
| P0-02 | **공개용 기본 `EventType` 시드 + 종별 기본 프리셋 템플릿** 확정. **시작 3개(`isStarter`)와 척도(`scaleType`) 지정 포함** | 사람 + 에이전트 | `docs/seed-event-types.md`. **개인 사용 패턴이 아니라 일반 공개 기본값** (§7.2) |
...
| P0-06 | 저장소명·npm 패키지명 충돌 확인 | 사람 | GitHub `eigger/kibble`, npm 스코프 |
...
| P0-09 | **파싱 벤치마크 세트 구축** — 기존 일지에서 실제 문장 100개를 뽑아 (시각·수량·타입) 정답을 붙인다 | 사람 | 규칙 기반 파서의 목표치를 정할 근거. **개인 기록이므로 저장소에 커밋하지 않는다** — 로컬 픽스처로 두고 `.gitignore`. 공개용 테스트는 합성 문장으로 별도 작성 |
