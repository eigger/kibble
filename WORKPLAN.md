...
| P0-07 | `PROJECT.md §4` 스키마 확정판 (§3.2 표 + §7 반영) | 에이전트 | 갱신 완료 |
...
| **다중 기기 작업 중 계획서가 갈라진다** | 집/회사에서 서로 다른 전제로 작업 | `WORKPLAN.md`가 단일 진실 원천. 결정이 바뀌면 **코드보다 먼저** 이 문서를 고치고 push ([`CLAUDE.md`](CLAUDE.md)) |
...
17. 커밋 접두사 `feat:` / `fix:` / `chore:` / `docs:`. 스키마 변경은 항상 마이그레이션 파일.
