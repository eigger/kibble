...
import { isRecordNotFoundError, isUniqueConstraintError } from "./prismaErrors.js";
...
describe("isRecordNotFoundError", () => {
  it("returns true for a Prisma P2025 record not found error", () => {
    const err = new Prisma.PrismaClientKnownRequestError("Record not found", {
      code: "P2025",
      clientVersion: "5.22.0",
    });
  ...
  it("returns false for other Prisma error codes", () => {
    ...
    expect(isRecordNotFoundError(err)).toBe(false);
  });
});
